package cloud.benchflow.experiment.config.docker.compose
...
object ServiceYamlProtocol extends DefaultYamlProtocol {
  ...
  implicit object EnvironmentV2YamlFormat extends YamlFormat[EnvironmentV2] {

    override def read(yaml: YamlValue): EnvironmentV2 = {
      yaml.asYamlObject.fields.get(YamlString("environment")).map {
        case YamlArray(variables) =>
          new EnvironmentV2(MutableMap(variables.map(_.convertTo[String]).map(EnvironmentVariable.apply).toSeq: _*))
        case _ => ???
      }
      .getOrElse(new EnvironmentV2(MutableMap.empty))
    }

    override def write(env: EnvironmentV2): YamlValue = {
      YamlObject(
          ...
          match {
            case Some(YamlArray(vars)) =>
                YamlObject(
                  YamlString("environment") ->
                    YamlArray(vars)
                ).convertTo[EnvironmentV2]
            case _ => EnvironmentV2(MutableMap.empty[String, String])
          }
